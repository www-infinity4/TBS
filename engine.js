(function (root) {
  ...
  function createSegments(block) {
    const runtime = Math.min(Number(block.movie.runtimeSeconds) || block.blockSeconds, block.blockSeconds);
    const segments = [];
    ...
    if (runtime < block.blockSeconds) {
      segments.push({kind:"station", title:"TBS station break — next program starts on schedule", videoId:"", cleared:true, sourceStart:0, stationStart:runtime, duration:block.blockSeconds-runtime});
    }
    return segments;
  }
  ...
  root.TBSEngine = { TIME_ZONE, stationParts, zonedToUtc, dateKey, createDaySchedule, createSegments, resolve };
})(window);
